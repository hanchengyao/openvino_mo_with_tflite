...
def apply_replacements(graph: Graph, replacements_type: list):
    ...
    if graph.graph['cmd_params'].framework == 'tflite':
        ...
        from tflite_frontend.tflite_parser import tflite_parser
        from tflite_frontend.tflite_nx_graph_generator import tflite_nx_graph_generator

        from tflite_frontend.tflite_extractor import tflite_op_extractor

        ordered_ops_list, data_dict, param_value_dict, model_input_dict, model_output_dict = tflite_parser(tflite_model)
