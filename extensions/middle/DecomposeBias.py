        ...
        return dict(
            nodes=[
                ('op', dict(kind='op', op=lambda op: op in ['Conv', 'ConvTranspose', 'Conv2D',
                                                            ...
                                                            'CONV_2D']))],  # [Mine] add this for tflite op: CONV_2D, 
                                                                            # the reason why there is no need to add an ertry for 'DEPTHWISE_CONV_2D'
                                                                            # is because now I make dw conv to use the same extractor as conv   
            edges=[]
        )

    @staticmethod
    def replace_pattern(graph: Graph, match: dict):
        # print("match conv-with-biasadd pattern!")
        node = match['op']
